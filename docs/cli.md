...
slay config init --relay-addr relay.example.com:443 --relay-public-key ~/.ssh/id_relay.pub --agent-tls private-ca --tls-dir ./slay-tls

# Use externally managed TLS files, such as a public CA certificate.
slay config init --relay-addr relay.example.com:443 --relay-public-key ~/.ssh/id_relay.pub --agent-tls external

# Local development only.
slay config init --relay-addr 127.0.0.1:4443 --relay-public-key ~/.ssh/id_relay.pub --agent-tls insecure
```
...
Keep `agent-ca.key` and `agent-relay.key` private. Do not copy `agent-ca.key` to agent machines.
